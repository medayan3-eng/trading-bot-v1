...
def render_sidebar():
    with st.sidebar:
        ...
        st.markdown("---")
        max_stocks = st.slider("Max stocks to scan", 50, 1584, 1584)
        st.markdown("---")
...
def render_stock_card(stock, bt_summary=None):
    ...
    breakout_conf    = stock.get('breakout_confirmed', False)
    breakout_level   = stock.get('breakout_level')
    vol_divergence   = stock.get('vol_divergence', False)
    vol_trend        = stock.get('vol_trend', 'neutral')
    accum_score      = stock.get('accum_score', 0)
    ...
    if breakout_conf:           badges.append(f'<span class="badge badge-green">🚀 BREAKOUT CONFIRMED{" $"+str(breakout_level) if breakout_level else ""}</span>')
    if vol_divergence:          badges.append('<span class="badge badge-red">⚠️ VOL DIVERGENCE</span>')
    if vol_trend == "rising":   badges.append('<span class="badge badge-green">📊 VOL RISING</span>')
    if accum_score >= 3:        badges.append('<span class="badge badge-blue">🏦 ACCUMULATION</span>')
